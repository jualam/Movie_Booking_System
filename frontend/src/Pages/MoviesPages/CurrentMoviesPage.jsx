...
const CurrentMoviesPage = () => {
    ...
    const fetchMovies = async () => {
      try {
        ...
        const response = await fetch(
          "http://localhost:5500/api/movies/current",
          {
            headers: {
  ...
  const handleBookNowClick = (e, movieId) => {
    e.preventDefault();
    e.stopPropagation();
    console.log("Book Now clicked for movieId:", movieId); // Debug log
    if (!movieId) {
        ...
        <div className="container mx-auto flex justify-between items-center">
          ...
          <nav className="hidden md:flex items-center space-x-6">
            ...
            <Link
              to="/browseCurrent"
              className={`hover:text-purple-400 transition-colors ${
                location.pathname === "/browseCurrent"
                  ? "text-purple-400 font-medium"
                  : ""
              }`}
            >
            ...
            <Link
              to="/browseUpcoming"
              className={`hover:text-purple-400 transition-colors ${
                location.pathname === "/browseUpcoming"
                  ? "text-purple-400 font-medium"
                  : ""
              }`}
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
          {movies.map((movie) => (
            ...
            >
              ...
              <div className="p-4">
                <h3
                  className="text-lg font-medium text-center h-8 truncate"
                  title={movie.title}
                >
                  {movie.title}
                </h3>
